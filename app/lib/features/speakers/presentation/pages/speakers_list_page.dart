...
import 'package:unleash_your_brave/features/speakers/presentation/widgets/speaker_card.dart';

enum _LoadStatus { loading, refreshing, success, offline, error }

class SpeakersListPage extends StatefulWidget {
  const SpeakersListPage({super.key, this.focusEventId});

  /// When opened from Home, prefer this edition’s speakers.
  final String? focusEventId;

  @override
  State<SpeakersListPage> createState() => _SpeakersListPageState();
}
...
class _SpeakersListPageState extends State<SpeakersListPage> {
  ...
  EventEntity? _event;
  List<SpeakerEntity> _speakers = const [];
  ...
  @override
  void didUpdateWidget(covariant SpeakersListPage oldWidget) {
  ...
  List<SpeakerEntity> get _visibleSpeakers {
    if (_searchQuery.trim().isEmpty) return _speakers;
    return _speakers
        .where((speaker) => speaker.matchesSearch(_searchQuery))
  ...
  Future<void> _load({required bool isRefresh}) async {
    ...
    try {
      ...
      final speakers =
          await sl<SpeakersRemoteDataSource>().list(eventId: event.id);
      if (!mounted) return;
      setState(() {
  ...
  List<SearchSuggestionItem> _suggestionsFor(String draft) {
    return _speakers
        .where((speaker) => speaker.matchesSearch(draft))
        .map(
          (speaker) => SearchSuggestionItem(
            id: speaker.id,
            title: speaker.name,
            subtitle: speaker.title.trim().isEmpty ? null : speaker.title.trim(),
          ),
        )
        .toList(growable: false);
  }
  ...
  Widget _buildBody(double sidePad) {
    final visible = _visibleSpeakers;
                  ...
                  children: [
                    ...
                    Text(
                      _event?.name.isNotEmpty == true
                          ? _event!.name
                          : 'All speakers linked to the current edition',
